    ...
    public partial class Backlinks : UserControl
    {
        ...
        public void GetBacklinks()
        {
            ...
            using (AppServices.WebcoseoDbContext context = new AppServices.WebcoseoDbContext())
            {
                ...
                {
                    WebsiteBacklinks_List.ItemsSource = context.Backlinks.Where(x => x.WebsiteId == GetWebsiteId)
                        .Skip(pageSize * (pageNumber - 1)).Take(pageSize).ToList();
                } else
                {
                    WebsiteBacklinks_List.ItemsSource = context.Backlinks.Where(x => x.WebsiteId == GetWebsiteId && x.Status == BacklinksStatus)
                        .Skip(pageSize * (pageNumber - 1)).Take(pageSize).ToList();
                }
            }
        }
        ...
        private void Backlinks_Search_Click(object sender, RoutedEventArgs e)
        {
            ...
            using (AppServices.WebcoseoDbContext context = new AppServices.WebcoseoDbContext())
            {
                var backlinksfound = context.Backlinks.Where(l => l.BacklinkUrl.ToLower().Contains(Backlinks_SearchBox.Text.ToLower()) && l.WebsiteId == GetWebsiteId);
        ...
        private void EditBacklink_Click(object sender, RoutedEventArgs e)
        {
            ...
            EditBacklink._instance.LoadDetails(idconv);
        ...
        public void Get_Total_Pages()
        {
            using (AppServices.WebcoseoDbContext context = new())
            {
                if (BacklinksStatus == "All")
                {
                    totalItems = context.Backlinks.Where(x => x.WebsiteId == GetWebsiteId).Count();
                }
                else
                {
                    totalItems = context.Backlinks.Where(x => x.WebsiteId == GetWebsiteId && x.Status == BacklinksStatus).Count();
                }
